...
0]
  ...
  def crosstab_filter
    if params["detail_filter"].nil?
      ...
      return "\"#{CrosstabObject.connection.quote_string(detail.name.downcase)}\" ilike '%#{CrosstabObject.connection.quote_string(params["value_filter"].to_s)}%'"
    end
  end
  ...
  def get_paginated_list(crosstab_query, h = {})
    ...
    if crosstab_count.to_i>0
      @paginator = Paginator.new self, crosstab_count.to_i, list_length, page_number(crosstab_query)
