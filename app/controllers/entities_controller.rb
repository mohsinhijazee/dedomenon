...
class EntitiesController < ApplicationController
  ...
  def entities_list
    ...
    @details = {}
    # Pick the details of this entity and populate them in a hash which is keyed
    # by the name of the detail.
    ...
    @paginator = Paginator.new self, Instance.count(:all, :conditions => "entity_id=#{@entity.id}"), 10, params[@list_id+"_page"]
    # List order is a has containing the information about the ordering.
    session["list_order"]||={}
    
    # The attempt here seems to pick the order.
    # If the list order is present but highlight paramter is not present
    if params["#{@list_id}_order"] and ! params["highlight"] or params["highlight"]==""
      order=CrosstabObject.connection.quote_string(params["#{@list_id}_order"].to_s)
    ...
    # Detail filter would ommit the fields
    # Here, if the detail filter is present, we wouuld find that detail
    if !params["detail_filter"].nil?
      detail = Detail.find params["detail_filter"]
      # Div class relates to the class of the table generated by the underlying view
      @div_class = "filtered"
      clause = "where \"#{detail.name.downcase}\" ilike '%#{CrosstabObject.connection.quote_string(params["value_filter"].to_s)}%'"
      separator = "and"
    else
    ...
    crosstab_filter = "#{clause}"
    if params["detail_filter"] or  params["format"]=="csv"
    ...
    crosstab_row =  CrosstabObject.connection.execute("select count(*) from #{crosstab_query} #{crosstab_filter}")[0]
    ...
    if crosstab_count.to_i>0
      ...
      query = "select * from #{crosstab_query}  #{crosstab_filter} order by \"#{order}\""
    ...
    response.headers["MYOWNDB_highlight"]=params["highlight"] if params["highlight"]
...
#      params[:id] = params[:entity_id]
      entities_list()
      #@json = records_to_json(@list, @ordered_fields)
      
#      respond_to do |format|
#        format.html { render :text => @json, :status => 200 }
...
#      end
  ...
  def show()
    view()
    #@json = record_to_json(@crosstab_object)
    
    # We are returning it from here
    #
    # FIXME: Should we do this now? Are these code lines are 
    # needed now?
#    respond_to do |format|
#      format.html  { render :text => @json, :status => 200 }
#      format.json  { render :json => @json, :status => 200 }
#    end
  ...
  #   POST /databases/:database_id/entities/:entity_id/instances
  # FIXME: For now, we stick to the format of the parameter passing as indicated
  #  as indicated in the EntitiesController#save_entity. For later, we should
  #  be able to do it like the following ways:
  #  
  #  Name=Raphe&Age=54
  #  
  #  which would yield a sepeate params[] entry for each of the field.
  #  Or if we want to create multiple records in a single go, we should be
  #  able to do like this:
  #  Name[0]=Mohsin&Age[0]=23&Name[1]=Raph&Age=99
  #  
  #  For now, you need to do it like this:
  #  
  # Name[0][value]=Mohsin&Age[0][value]=45
  # 
  # This would set the [id] cols nil and hence would give the code a chance to 
  # create a new instancne instead of trying to edit an existing one.
  # Its quit ugly but we'll change it later on.
  # DONE!
  def create()
    respond_to do |format|
      
      # We need to do the parameter swaping so that
      # save_entity can chew them easily
      params[:instance_id] = -1
      params[:form_id] = String.random(8)
      params[:entity] = params[:entity_id]
      
      begin
        entity = Entity.find(params[:entity_id])
      rescue ActiveRecord::RecordNotFound
        respond_to do |format|
          format.html { render :text => "Bad Request (Entity[#{params[:entity_id]}] not found)", :status => 400 and return }
          format.json { render :text => "Bad Request (Entity[#{params[:entity_id]}] not found)", :status => 400 and return }
        end
      end
      
      missing_details = verify_details_for_entity(entity)
      
      
      # if entity of the given ID was not found
      if !missing_details
        respond_to do |format|
        format.html { render :text => 'Bad Request (Entity of the given ID was not found)', :status => 400 and return }
        format.json { render :json => 'Bad Request (Entity of the given ID was not found)', :status => 400 and return }
        end
      end
      
      
      # If we have any missing details from the request parameters,
      # List them and return.
      if missing_details.length > 0
      
        str = 'Bad Request ('
        missing_details.each do |detail_name|
          str += detail_name + ', '
        end
        
        str.chop!
        str.chop!
        str += " missing from the request for the entity[#{entity.name}])"
          
        respond_to do |format|
          format.html { render :text => str, :status => 400 and return }
          format.json { render :json => str, :status => 400 and return }
        end
      ...
      begin
        ...
        format.html { render :text => "Failed creating instance<br>#{e.to_s}", :status => 400 and return }
        format.json { render :json => "Failed creating instance<br>#{e.to_s}", :status => 400 and return }
      end
      ...
      if saved
        ...
        format.html { render :text => 'NOT SAVED', :status => 400 }
        format.json { render :json => 'NOT SAVED', :status => 400 }
      end
    end  
  end
  ...
  #   
  #  *REST_API*
  #     PUT /entities/:entity_id/instances/:id
  #     PUT database/:database_id/entites/:entity_id/instances/:id
  # DONE!
  def update()
    ...
    respond_to do |format|
      begin
        ...
        format.html { render :text => 'Update Failed', :status => 400 and return }
        format.json { render :json => 'Update Failed', :status => 400 and return }
      end
      
      if updated
        format.html { render :text => 'OK', :status => 200 and return }
        format.json { render :json => 'OK', :status => 200 and return }
      else
        format.html { render :text => 'Failed', :status => 400 and return }
        format.json { render :json => 'Failed', :status => 400 and return }
      end
    end
  end
  ...
  def unlink
    ...
    respond_to do |format|
      format.html { render_component(:controller => "entities", :action => "related_entities_list", :id => params[render_id],:params => { :relation_id => params["relation_id"] , :type => type })  }
      format.json { render :json => 'OK', :status => 200 }
    end
  end
  ...
  def list_available_for_link
    ...
    @details = {}
    @entity.details.each do |d|
      @details[d.name.downcase]=d
    end
    separator = " where "
    if !params["detail_filter"].nil?
      detail = Detail.find params["detail_filter"]
      filter_clause = "#{separator} \"#{detail.name.downcase}\" ilike '%#{CrosstabObject.connection.quote_string(params["value_filter"].to_s)}%'"
      separator = " and "
    end
    link_filter = "#{separator} id not in (select #{related_id} from links where relation_id = #{@relation.id} #{other_side_type_filter})"
    separator = " and "
    ...
    if crosstab_result
      ...
      @not_in_list_view  = crosstab_result[:not_in_list_view]
      @ordered_fields   = crosstab_result[:ordered_fields]
      count_row = CrosstabObject.connection.execute("select count(*) from #{crosstab_query}  #{filter_clause}  #{link_filter}")[0]
      count = count_row[0] ? count_row[0] : count_row['count']
      @paginator = Paginator.new self, count.to_i , 10, params[@list_id+"_page"]
      limit, offset = @paginator.current.to_sql
      @list = CrosstabObject.find_by_sql("select * from #{crosstab_query} #{filter_clause}  #{link_filter}  order by \"#{order}\" limit #{limit} offset #{offset}")
    else
      @list=[]
    end


#    #redirect to the view if there is no entity available (except if we filtered)
#    if @instances.length==0 and filter.nil?
#			flash["error"]  = "error_no_record_eligible_for_link"
#      redirect_to :action => "view", :id => params[self_id]
#    end
  ...
  def link_entities(parent,relation,child)
    ...
    respond_to do |format|
      if params["parent_id"]
        if params["embedded"]
          format.html { redirect_to :controller => "entities", :action => "related_entities_list", :id => params["parent_id"],  :relation_id => relation.id, :type => "children", :highlight => child.id }
          format.json { render :json => 'OK', :status => 200 }
        else
          format.html { redirect_to(:action => "view", :id=>parent.id) }
          format.json { render :json => 'OK', :status => 200 }
        end
      elsif params["child_id"]
        if params["embedded"]
          format.html { redirect_to :controller => "entities", :action => "related_entities_list", :id => params["child_id"],  :relation_id => relation.id, :type => "parents", :highlight => parent.id }
          format.json { render :json => 'OK', :status => 200 }
        else
          format.html { redirect_to(:action => "view", :id=>child.id) }
          format.json { render :json => 'OK', :status => 200 }
        end
      end
    end
  ...
  def related_entities_list
    ...
    if !params["detail_filter"].nil?
      detail = Detail.find params["detail_filter"]
      @div_class = "filtered"
      clause = "and #{detail.name} ilike '%#{CrosstabObject.connection.quote_string(params["value_filter"].to_s)}%'"
    else
      @div_class = "unfiltered"
      clause =""
    end
    ...
    crosstab_filter = " where id in (#{ids_to_keep}) #{clause} "
    ...
    if ids_to_keep.length>0
      count_row =  CrosstabObject.connection.execute("select count(*) from #{crosstab_query} #{crosstab_filter}")[0]
    ...
    if crosstab_count.to_i > 0
      ...
      highlight_value_row =  CrosstabObject.connection.execute("select #{order} from #{crosstab_query} #{crosstab_filter} #{separator} id=#{CrosstabObject.connection.quote_string(params["highlight"].to_s)}")[0]
      highlight_value = highlight_value_row[0] ? highlight_value_row[0] : highlight_value_row['id']
      highlight_count_row =  CrosstabObject.connection.execute("select count(*) from #{crosstab_query} #{crosstab_filter} #{separator} #{order}<'#{CrosstabObject.connection.quote_string(highlight_value.to_s)}'")[0]
      highlight_count = highlight_count_row[0] ? highlight_count_row[0] : highlight_count_row['count']
      ...
      @paginator = Paginator.new self, crosstab_count.to_i, 10, page_number
      limit, offset = @paginator.current.to_sql
      @list = CrosstabObject.find_by_sql("select * from #{crosstab_query}  #{crosstab_filter} order by #{order} limit #{limit} offset #{offset}")
    else
      @list = []
    end
...
#    @list=@list[@paginator.current_page.first_item-1..@paginator.current_page.last_item-1]
